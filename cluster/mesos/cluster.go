...
func (c *Cluster) Info() [][2]string {
	...
	for _, offer := range offers {
		info = append(info, [2]string{"  Offer", offer.SlaveId.GetValue() + ">>" + offer.Id.GetValue()})
		for _, resource := range offer.Resources {
			if resource.GetRevocable() != nil {
				info = append(info, [2]string{"   └ " + resource.GetName() + "(Revocable)", formatResource(resource)})
			} else if c.mesosRole != defaultMesosRole && *(resource.Role) == c.mesosRole {
				info = append(info, [2]string{"   └ " + resource.GetName() + "(Reserved)", formatResource(resource)})
			} else {
				info = append(info, [2]string{"   └ " + resource.GetName(), formatResource(resource)})
			}
		}
		engLabels := c.agents[offer.SlaveId.GetValue()].engine.Labels
		labels := make([]string, 0, len(engLabels))
...
func (c *Cluster) LaunchTask(t *task.Task) bool {
	...
	t.Build(n.ID, c.agents[n.ID].offers, c.mesosRole, useRevocableResources)
